...
import com.squareup.timessquare.TakeTestDescriptor;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    multi.setOnClickListener(new OnClickListener() {
      ...
      public void onClick(View v) {
		...
		ArrayList<TakeTestDescriptor> takeDates = new ArrayList<TakeTestDescriptor>();
		for (int i = 0; i < 5; i++) {
			testDay.add(Calendar.DAY_OF_MONTH, 3);
			takeDates.add(new TakeTestDescriptor().setTestDay(testDay.getTime()).setTensionState(TensionState.values()[i+1]));
		}
		calendar.init(new Date(), nextYear.getTime()) //
				.inMode(SelectionMode.SINGLE) //
				.withTensionHiglightDates(takeDates);
      }
    });
