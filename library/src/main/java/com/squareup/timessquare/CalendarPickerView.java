...
public class CalendarPickerView extends ListView {
  ...
  final List<TakeTestDescriptor> takeTestDate = new ArrayList<TakeTestDescriptor>();
  ...
  public class FluentInitializer {
    ...
    public FluentInitializer withTensionHiglightDates(Collection<TakeTestDescriptor> takeDates){
    	tensionHighlightDates(takeDates);
    	return this;
    }
  ...
  public void tensionHighlightDates(Collection<TakeTestDescriptor> takeTestDates){
	  Iterator<TakeTestDescriptor> takeTestIterator = takeTestDates.iterator();
	  while (takeTestIterator.hasNext()){
		  TakeTestDescriptor descriptor = takeTestIterator.next();
		  if(descriptor == null)return;
		  
		  Date date = descriptor.getTestDay();
		  validateDate(date);
		  
		  MonthCellWithMonthIndex monthCellWithMonthIndex = getMonthCellWithIndexByDate(date);
	      if (monthCellWithMonthIndex != null) {
	    	  Calendar newlyTakeTestDate = Calendar.getInstance();
	    	  newlyTakeTestDate.setTime(date);
	    	  MonthCellDescriptor cell = monthCellWithMonthIndex.cell;
	    	  takeTestDate.add(descriptor);
	    	  cell.setTakeTestDescriptor(descriptor);
	      }
	  }
  ...
  List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month, Calendar startCal) {
    ...
    while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month.getYear()) //
      ...
      for (int c = 0; c < 7; c++) {
        ...
        // Daily average tension level 
        // checked the take test day or not 
        TakeTestDescriptor descriptor = containsTakeTestDate(takeTestDate, cal);
        
		weekCells.add(new MonthCellDescriptor(date, isCurrentMonth,
				isSelectable, isSelected, isToday, isHighlighted,
				value, rangeState, descriptor));
        cal.add(DATE, 1);
      }
    }
    return cells;
  ...
  private static TakeTestDescriptor containsTakeTestDate(List<TakeTestDescriptor> selectedCals, Calendar cal) {
	  TakeTestDescriptor returnDescriptor = null;
	  Iterator<TakeTestDescriptor> iterator = selectedCals.iterator();
	  while(iterator.hasNext()){
		  TakeTestDescriptor descriptor = iterator.next();
		  if (sameDate(cal,descriptor.getTestDayCalendar())) {
